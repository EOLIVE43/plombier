...
/* ============================================================
   MEGA MENU : auto-flip à droite si débordement viewport
   ============================================================ */
(function() {
  function adjustDropdowns() {
    if (window.innerWidth < 992) return; // mobile : géré différemment
    ...
    dropdowns.forEach(dd => {
      ...
      // Reset
      dd.classList.remove('dropdown-flip-right');
      ...
      // Si le panel partant à gauche du parent dépasserait à droite, on flip
      if (ddRect.left + panelWidth > viewportWidth - 16) {
        dd.classList.add('dropdown-flip-right');
      }
    });
  }
  ...
  window.addEventListener('resize', () => {
    ...
    resizeTimer = setTimeout(adjustDropdowns, 100);
  });
  
  // Recalculer aussi quand un dropdown est survolé (au cas où)
  document.querySelectorAll('.has-dropdown').forEach(dd => {
    dd.addEventListener('mouseenter', () => {
      if (!dd.classList.contains('dropdown-flip-right')) {
        const panel = dd.querySelector('.dropdown-panel');
        if (panel) {
          const ddRect = dd.getBoundingClientRect();
          const panelWidth = panel.offsetWidth || 340;
          if (ddRect.left + panelWidth > window.innerWidth - 16) {
            dd.classList.add('dropdown-flip-right');
          }
        }
      }
